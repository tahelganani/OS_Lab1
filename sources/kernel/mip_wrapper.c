...
struct mpi_message* mpi_find_message_from_sender(pid_t receiver, pid_t sender) {
    struct list_head* pos;
    struct mpi_message* msg;
    struct task_struct* task = find_task_by_pid(receiver);
    if(!task) return NULL;

    list_for_each(pos, &task->mpi_queue) {
        msg = list_entry(pos, struct mpi_message, list);
        if(msg->sender_pid == sender) return msg;
    }
    return NULL;
}
...
asmlinkage int sys_mpi_receive(pid_t pid, char *message, ssize_t message_size) {
    ...
    ssize_t num_bytes;
    if(!message || message_size < 1) {return -EINVAL;}
    if(!current->mpi_registered) {return -EPERM;}
    msg = mpi_find_message_from_sender(current->pid, pid);
    if(!msg) {return -EAGAIN;}
    num_bytes = msg->size < message_size ? msg->size : message_size;
    if(copy_to_user(message, msg->data, num_bytes)) {return -EFAULT;}
    list_del(&msg->list);
    mpi_message_free(msg);
    return num_bytes;
}
...
asmlinkage int sys_mpi_unregister(int mpi_gid) {
    if(mpi_gid < -1 || mpi_gid ==0) {
        return -EINVAL;
    }
    if(mpi_gid == -1) {
        mpi_clear_groups(current->pid);
        return 0;
    }
    mpi_clear_messages_by_group(current->pid, mpi_gid);
    mpi_remove_group(current->pid, mpi_gid);
    return 0;
}
